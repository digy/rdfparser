...
public class TestRDFSExpander {
	...
	public void test_impicitResources() {
		RDFGraph right_data = new RDFGraph(new LinkedList<RDFTriplet>(test_data.basic1));
		RDFSExpander exp = new RDFSExpander();
		exp.expand(right_data);
		
		RDFTriplet _res1 = new RDFTriplet(
				new RDFUri("http://semantic-web-book.org/uri"),
				new RDFUri(rdfns + "rdf:type"),
				new RDFUri(rdfsns + "rdfs:Resource"));
	...
	public void test_litera() {
		RDFGraph right_data = new RDFGraph(new LinkedList<RDFTriplet>(test_data.basic2));
		RDFSExpander exp = new RDFSExpander();
		exp.expand(right_data);
		RDFTriplet _lit = new RDFTriplet(
	...
	public void test_domain() {
		RDFGraph right_data = new RDFGraph(new LinkedList<RDFTriplet>(test_data.basic1));
		right_data.addTriplet_uri("http://www.example.org/publishedBy",
				rdfsns + "rdfs:domain",
				"ex:book");
		RDFSExpander exp = new RDFSExpander();
		exp.expand(right_data);
	...
	public void test_subprop() {
		RDFGraph right_data = new RDFGraph(new LinkedList<RDFTriplet>(test_data.basic1));
		right_data.addTriplet_uri("http://www.example.org/publishedBy", rdfsns + "rdfs:subPropertyOf", "ex:owned");
		RDFSExpander exp = new RDFSExpander();
		exp.expand(right_data);
		RDFTriplet _subProp = new RDFTriplet(
				new RDFUri("http://semantic-web-book.org/uri"),
				new RDFUri("ex:owned"),
				new RDFUri("http://crcpress.com/uri"));
